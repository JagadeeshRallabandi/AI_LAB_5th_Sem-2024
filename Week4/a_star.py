...
class MissionariesAndCannibals:
        ...
        while open_list:
            ...
            for new_state in self.get_neighbors(current_state):
                if new_state not in closed_set:
                    new_cost = cost + 1
                    total_cost = new_cost + self.heuristic(new_state)
                    heapq.heappush(open_list, (total_cost, new_cost, new_state, path + [current_state]))
    ...
    def solve(self):
        ...
        if path is not None:
            ...
            for state in path:
                print(state)
            print(f"The number of moves is {len(path) - 1}")  # Subtract 1 because the first state is the start state
        else:
